...
static int validate_pe(BuildContext* ctx, ConsoleContext* console) {
...
static int read_input_file(BuildContext* ctx, ConsoleContext* console)
{
...
static int update_resources(BuildContext* ctx, CryptoContext* crypto, ConsoleContext* console)
{
    ...
    if (!UpdateResource(ctx->hUpdateRes, RT_RCDATA, MAKEINTRESOURCE(132),
    ...
    // Update key resource
    if (!UpdateResource(ctx->hUpdateRes, RT_RCDATA, MAKEINTRESOURCE(133),
        MAKELANGID(LANG_NEUTRAL, SUBLANG_NEUTRAL), crypto->obfuscatedKey, 32)) {
    ...
    if (!UpdateResource(ctx->hUpdateRes, RT_RCDATA, MAKEINTRESOURCE(134),
        MAKELANGID(LANG_NEUTRAL, SUBLANG_NEUTRAL), crypto->obfuscatedNonce, 24)) {
...
int main(int argc, char* argv[])
{
    ...
    crypto.obfuscatedKey = (uint8_t*)malloc(32);
    crypto.obfuscatedNonce = (uint8_t*)malloc(24);
    ...
    if (!read_input_file(&ctx, &console) || !validate_pe(&ctx, &console))
    {
        goto cleanup;
    }
    ...
    int hkdf_r1 = hkdfReset(&hkdfCTX, SHA256, crypto.salt, 16);
    int hkdf_r2 = hkdfInput(&hkdfCTX, crypto.hash, 32);
    int hkdf_r3 = hkdfResult(&hkdfCTX, crypto.prk, (const uint8_t*)"obfuscation-context", 
                    (int)strlen("obfuscation-context"), crypto.okm, 80);
    ...
    chacha20poly1305_encrypt(&obfuscationCtx, crypto.key, crypto.obfuscatedKey, 32);
    chacha20poly1305_encrypt(&obfuscationCtx, crypto.nonce, crypto.obfuscatedNonce, 24);

    // Update resources in final.exe
    if (!update_resources(&ctx, &crypto, &console))
    {
        goto cleanup;
    }
